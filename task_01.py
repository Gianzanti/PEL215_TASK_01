...
class MyRobot:
    ...
    def initSensors(self):
        ...
        # self.front_left_wheel_sensor = self.me.getDevice("front left wheel sensor")
        # self.front_right_wheel_sensor = self.me.getDevice("front right wheel sensor")
        # self.back_left_wheel_sensor = self.me.getDevice("back left wheel sensor")
        # self.back_right_wheel_sensor = self.me.getDevice("back right wheel sensor")

        for sensor in self.front_distance_sensors:
    ...
    def readSensors(self):
        self.front_sensor_values = [
            sensor.getValue() for sensor in self.front_distance_sensors
        ]
    ...
    def detectFrontWall(self):
        max_sensor = max(
            self.front_sensor_values[3],
            self.front_sensor_values[4],
        )
        print(f"Max Front sensor: {max_sensor}")

        currentDistanceFromWall = 1024 - max_sensor
        print(f"Current distance from front wall: {currentDistanceFromWall}")

        # error = self.pid_f_wall.update(currentDistanceFromWall)
        # print(f"Error: {error}")
